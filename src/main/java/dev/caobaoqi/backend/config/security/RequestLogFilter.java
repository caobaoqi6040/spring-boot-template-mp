...
import dev.caobaoqi.backend.config.SnowflakeIdGenerator;
...
public class RequestLogFilter extends OncePerRequestFilter {
	...
	public void logRequestStart(HttpServletRequest request) {
		...
		request.getParameterMap().forEach((k, v) -> {
			try {
				object.put(k, v.length > 0 ? v[0] : null);
			} catch (JSONException e) {
				throw new RuntimeException(e);
			}
		});
		Object id = request.getAttribute(ATTR_USER_EMAIL);
		if (id != null) {
			User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
			log.info("请求 URL: \"{}\" ({}) | 远程 IP 地址: {} │ 身份: {} (UID: {}) | 角色: {} | 请求参数列表: {}",
				request.getServletPath(), request.getMethod(), request.getRemoteAddr(),
				user.getUsername(), id, user.getAuthorities(), object);
		} else {
			log.info("请求 URL: \"{}\" ({}) | 远程 IP 地址: {} │ 身份: 未验证 | 请求参数列表: {}",
